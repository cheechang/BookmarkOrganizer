...
找出三种重复：
- **完全重复**：URL 完全相同的书签
- **规范化重复**：路径相同但查询参数不同的书签（比如同一文档的两个链接，只有 `timestamp` 参数不同）
- **相似重复**：同域名下标题和 URL 路径高度相似的书签（比如 "React - 官方文档" 和 "React Docs"）
...
- 相似度算法基于 Levenshtein 距离（编辑距离），同时比较标题和 URL 路径并加权平均（标题 60%、URL 40%），阈值可在设置页面调整（默认 80%）
